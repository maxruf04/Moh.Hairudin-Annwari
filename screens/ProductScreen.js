    ...
    <View className="flex-1">
      ...
      <SafeAreaView className="space-y-4 flex-1">
        ...
        <View className="px-4 space-y-2">
          <Text style={{color: themeColors.text}} className="text-lg font-bold">Ukuran Kopi</Text>
          <View className="flex-row justify-between">
            <TouchableOpacity 
             onPress={()=> setSize('small')}
             style={{backgroundColor: size=='small'? themeColors.bgLight: 'rgba(0,0,0,0.07)'}} 
             className="p-3 px-8 rounded-full">
              <Text className={size=='small'? "text-white": "text-gray-700"}>Kecil</Text>
            </TouchableOpacity>
            <TouchableOpacity 
             onPress={()=> setSize('medium')}
             style={{backgroundColor: size=='medium'? themeColors.bgLight: 'rgba(0,0,0,0.07)'}}
              className="p-3 px-8 rounded-full">
              <Text className={size=='medium'? "text-white": "text-gray-700"}>Sedang</Text>
            </TouchableOpacity>
            <TouchableOpacity 
             onPress={()=> setSize('large')}
             style={{backgroundColor: size=='large'? themeColors.bgLight: 'rgba(0,0,0,0.07)'}} 
             className="p-3 px-8 rounded-full">
              <Text className={size=='large'? "text-white": "text-gray-700"}>Besar</Text>
            </TouchableOpacity>
          </View>
        </View>
      ...
      <View className={`space-y-3 ${ios? 'mb-6': 'mb-3'}`}>
          ...
          <View className="flex-row justify-between px-4">
            ...
            <TouchableOpacity 
              ...
              <Text className="text-center text-white text-base font-semibold">Beli Sekarang</Text>
            </TouchableOpacity>
          </View>
        </View>
